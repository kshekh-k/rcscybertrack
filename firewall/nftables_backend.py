...
class NftablesBackend(FirewallBackend):
    ...
    def _build_cmd(self, nft_args: List[str]) -> List[str]:
        ...
        # Root can execute nft directly.
        if hasattr(os, "geteuid") and os.geteuid() == 0:
            return base_cmd

        # CyberTrack production service is granted CAP_NET_ADMIN by
        # systemd. Prefer direct nft execution so the service does not
        # depend on sudo when operating with its network capability.
        if self._has_net_admin_capability():
            return base_cmd

        # Development/unprivileged fallback.
    ...
    def _has_net_admin_capability() -> bool:
        """Return True when the current process has CAP_NET_ADMIN.

        /proc/self/status is used instead of invoking external capability
        tools, keeping the backend lightweight and deterministic.
        """
        try:
            with open("/proc/self/status", "r", encoding="utf-8") as fh:
                for line in fh:
                    if line.startswith("CapEff:"):
                        ...
                        # Linux capability number 12 = CAP_NET_ADMIN.
                        return bool(effective & (1 << 12))
    ...
    def validate(self, policy: Dict, rules: List[Dict], nat: Optional[Dict] = None) -> Tuple[bool, str]:
        try:
            compiled = compiler.compile(policy, rules, nat=nat)
        except Exception as e:
    ...
    def compile(self, policy: Dict, rules: List[Dict], nat: Optional[Dict] = None) -> str:
        return compiler.compile(policy, rules, nat=nat)

    def apply(self, policy: Dict, rules: List[Dict], nat: Optional[Dict] = None) -> Tuple[bool, str]:
        valid, msg = self.validate(policy, rules, nat=nat)
        if not valid:
            return False, f"Pre-apply validation failed: {msg}"

        compiled = self.compile(policy, rules, nat=nat)

        if not self.nft_binary:
            return True, "Dry-run simulated apply successful (nft binary not present)"

        try:
            # Remove only CyberTrack-owned tables before applying the
            # newly compiled configuration. Do NOT flush the complete
            # nftables ruleset because other applications (e.g. Docker)
            # may own their own tables.
            for family, table in (
                ("ip", "rcs_cybertrack_nat"),
                ("inet", "rcs_cybertrack"),
            ):
                delete_cmd = self._build_cmd(
                ...
                if delete_res.returncode != 0:
                    err_msg = delete_res.stderr.strip()

                    # Missing table is harmless; any other error is real.
                    if "No such file or directory" not in err_msg:
                        return False, (
                            f"Failed to remove existing CyberTrack table "
                            f"{family} {table}: {err_msg}"
                        )

            # Apply the complete newly compiled configuration exactly once.
            ...
            if res.returncode != 0:
                ...
                if (
                    "password is required" in err_msg
                    or "Operation not permitted" in err_msg
                    or "Permission denied" in err_msg
                ):
                    return False, (
                        f"Privileged nftables execution failed: "
                        f"sudo passwordless rule for '{self.nft_binary}' "
                        f"is not configured or netlink access was denied. "
                        f"Error: {err_msg}"
                    )
...
class FakeNftExecutor(FirewallBackend):
    ...
    def apply(self, policy: Dict, rules: List[Dict], nat: Optional[Dict] = None) -> Tuple[bool, str]:
        ...
        self.active_ruleset = self.compile(policy, rules, nat=nat)
