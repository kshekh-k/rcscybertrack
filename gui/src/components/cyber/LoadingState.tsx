...
export const TableSkeleton: React.FC<{ rows?: number }> = ({ rows = 5 }) => {
  return (
    <div className="space-y-3">
      <div className="h-10 bg-app-bg border border-border-subtle rounded-lg animate-pulse" />
      {Array.from({ length: rows }).map((_, i) => (
        <div key={i} className="h-12 bg-surface/50 border border-border-subtle rounded-lg animate-pulse" />
      ))}
    </div>
  )
}
