...
export const CapabilityNotice: React.FC<CapabilityNoticeProps> = ({
  ...
  return (
    <div
      className={`bg-indigo-950/20 border border-indigo-500/30 rounded-xl p-4 my-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs text-indigo-200/90 shadow-sm ${className}`}
    >
      <div className="flex items-start gap-3">
        <div className="p-2 rounded-lg bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 shrink-0 mt-0.5 sm:mt-0">
        ...
        <div>
          ...
          <p className="mt-1 text-slate-300 leading-relaxed text-[11px] max-w-2xl">{description}</p>
        </div>
      </div>

      <div className="flex items-center gap-2 font-mono text-[11px] bg-slate-900/80 px-3 py-1.5 rounded-lg border border-slate-800 shrink-0">
